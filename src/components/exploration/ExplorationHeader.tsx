...
export function ExplorationHeader() {
  ...
  return (
    <header className="h-14 border-b border-border-subtle/30 bg-bg-base/60 backdrop-blur-md sticky top-0 z-30 transition-colors flex-shrink-0">
      <div className="max-w-7xl h-full mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <div className="w-7 h-7 rounded-md bg-accent-subtle/40 border border-accent-primary/20 flex items-center justify-center text-accent-primary group-hover:border-accent-primary transition-colors">
          ...
          <div className="flex items-center gap-1 pl-3 border-l border-border-subtle/50">
            <button
              ...
              className={`px-2.5 py-1 rounded text-xs font-mono flex items-center gap-1.5 transition-all ${
                canGoBack
                  ? 'text-fg-primary hover:bg-bg-surface hover:text-accent-primary border border-border-strong cursor-pointer shadow-2xs'
                  : 'text-fg-subtle opacity-30 cursor-not-allowed border border-transparent'
              }`}
            ...
            <button
              ...
              id="nav-reset-btn"
              onClick={reset}
              className="px-2.5 py-1 rounded text-xs font-mono text-fg-secondary hover:text-fg-primary hover:bg-bg-surface border border-border-subtle/50 flex items-center gap-1.5 transition-all cursor-pointer"
        ...
        {/* Right: Active Entity Indicator (No Light/Dark buttons) */}
        <div className="flex items-center gap-3">
          {focusedEntity && (
            <div className="flex items-center gap-2 text-xs font-mono text-fg-muted">
              <span className="w-1.5 h-1.5 rounded-full bg-accent-primary animate-pulse" />
              <span className="text-fg-subtle hidden sm:inline">Focus:</span>
              <span className="text-accent-primary font-medium tracking-wide truncate max-w-[180px]">
                {focusedEntity.name}
              </span>
            </div>
          )}
